...
from ddpg_learning import Agent
...
def simulate(vehicle_type="otter", dock="dummy_dock", sample_time=0.1, seconds_to_simulate=120, visualize=False, print_information=False, starting_position="random", movement_method="dq_agent", movement_model=None):
    ...
    actions_taken = np.zeros([1, 2], dtype=np.int32)
...
def main():
    ...
    agent = Agent(alpha=0.000025, beta=0.00025, input_dims=[NUMBER_OF_INPUTS], tau=0.001, env=None,
    ...
    for starting_position in starting_positions:
        ...
        for simulation_number in range(number_of_simulations):
            ...
            simulation_actions_taken, simulation_vehicle_observations, simulation_vehicle_next_observations, simulation_rewards, simulation_dones = simulate(starting_position=starting_position, movement_method="ddpg_agent", movement_model=agent)
            ...
            for n in range(len(simulation_actions_taken)):
                agent.remember(simulation_vehicle_observations[n, :], simulation_actions_taken[n], simulation_rewards[n], simulation_vehicle_next_observations[n, :], simulation_dones[n])
                agent.learn()
