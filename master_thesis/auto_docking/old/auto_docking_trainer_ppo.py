...
def set_vehicle_thrusters(vehicle="otter", method="random", mover=None, inputs=None):
    ...
    if vehicle == "otter":
        if method == "random":
        ...
        elif method == "ddpg_agent":
            action = mover.choose_action(inputs)
            thruster_configuration = 100*np.tanh(action)
            #thruster_configuration = action

        elif method == "ppo_agent":
            action, prob, val = mover.choose_action(inputs)
            ...
            if action == 0: # Go forward
                ...
                thruster_configuration[0] = 100
                thruster_configuration[1] = -100
            return action, thruster_configuration, prob, val
...
def simulate(vehicle_type="otter", dock="dummy_dock", sample_time=0.01, seconds_to_simulate=120, visualize=False, print_information=False, starting_position="random", movement_method="dq_agent", movement_model=None):
    ...
    # Sets out some obstacles (the corners of the dock)
    obstacles = np.array([[23.75, -26.25],
                         [28.75, -31.25],
                         [31.25, -28.75],
                         [26.25, -23.75]])
    ...
    for t in range(seconds_to_simulate):
        ...
        distance_to_closest_obstacle = np.inf
        for obstacle in obstacles:
            d = np.hypot(eta[1]-obstacle[0], eta[0]-obstacle[1])
            if d < distance_to_closest_obstacle:
                distance_to_closest_obstacle = d
                angle_to_closest_obstacle = smallest_signed_angle_between(0, np.arctan2(obstacle[1]-eta[0], obstacle[0]-eta[1]) - pi/2 - vehicle_yaw)
        ...
        observation = np.array([nu[0], # surge
                                ...
                                distance_to_closest_obstacle, # distance to closest obstacle
                                angle_to_closest_obstacle
                                ], dtype=np.float32)

        # Feature scaling
        feature_scaling_array = np.array([1, 0.1, np.hypot(27.5, -27.5), pi, pi, np.hypot(27.5, -27.5), pi])
        #feature_scaling_array = np.array([1, 1, 1, 1, 1])
        ...
        elif movement_method == "ddpg_agent":
            action_taken, u_control = set_vehicle_thrusters(method=movement_method, mover=movement_model, inputs=observation)
        elif movement_method == "ppo_agent":
            action_taken, u_control, prob, val = set_vehicle_thrusters(method=movement_method, mover=movement_model, inputs=observation)
        ...
        #actions_taken = np.r_[actions_taken, [action_taken]]
        ...
        if t == seconds_to_simulate-1: # Checks if we are on the last time step
            ...
            if distance_between_vehicle_and_dock < 1: # Adds reward for angle if close enough
                reward += 50*np.exp(-abs(angular_difference_between_vehicle_and_dock)/pi)
            done = True
    ...
    return actions_taken, vehicle_observations, vehicle_next_observations, rewards, dones, vals, probs
...
def main():
    ...
    n_epochs = 4
    alpha = 0.0003
    agent = Agent(n_actions=5, batch_size=batch_size, alpha=alpha, n_epochs=n_epochs, input_dims=[NUMBER_OF_INPUTS])
    ...
    for starting_position in starting_positions:
        ...
        for simulation_number in range(number_of_simulations):
            ...
            #starting_position = np.random.choice(starting_positions)

            simulation_actions_taken, simulation_vehicle_observations, simulation_vehicle_next_observations, simulation_rewards, simulation_dones, simulation_vals, simulation_probs = \
            simulate(starting_position=starting_position, movement_method="ppo_agent", movement_model=agent)
            ...
            all_final_rewards.append(simulation_rewards[-1])
            ...
            for n in range(len(simulation_actions_taken)):
                agent.remember(simulation_vehicle_observations[n, :], simulation_actions_taken[n], simulation_probs[n], simulation_vals[n], simulation_rewards[n], simulation_dones[n])
                agent.learn()

            print("\n")

        plt.plot(np.linspace(0, len(all_final_rewards), num=len(all_final_rewards)), all_final_rewards)
        plt.xlabel("Simulation number")
        plt.ylabel("Final reward")
        plt.xlim([0, len(all_final_rewards)])
        plt.ylim([0, 100])
        plt.title("PPO")
        plt.show()
        exit()
